...
# The list was narrowed on 2026-09-11. It used to forbid the words "zenodo", "integration" and
# "archived" outright, which was right when the only such sentence was pipeline instruction and is
# wrong now that a published archive with an identifier exists: naming the archive and its
# identifier is what a reader of a release needs. What stays forbidden is the mechanics - what has
# to be enabled, what a draft does not trigger, which page to click, which token to revoke.
MACHINERY = ["must be enabled", "never archived", "binary asset", "draft release", "edit page",
             "access token", "revoke", "integration watches", "sync_draft", "deposit.sh",
             "before the release is published", "not published as a"]
...
# ---- CITATION.cff must be a citation file the format can read ----
# The file invented a `versions:` key and declared cff-version 1.3.0, neither of which exists:
# checked against the official schema (github.com/citation-file-format, fetched 2026-09-11), whose
# additionalProperties is false, so an invented key is a validation error and the invented history
# block would be dropped by every converter. Keys are now prose comments, and the check fails on
# any top-level key outside the schema's list.
CFF_KEYS = {"abstract", "authors", "cff-version", "commit", "contact", "date-released", "doi",
            "identifiers", "keywords", "license", "license-url", "message", "preferred-citation",
            "references", "repository", "repository-artifact", "repository-code", "title", "type",
            "url", "version"}
cff_file = EV.parent / "CITATION.cff"
if cff_file.exists():
    cff_text = cff_file.read_text(errors="replace")
    cff_top = [m.group(1) for m in re.finditer(r"^([A-Za-z][A-Za-z-]*):", cff_text, re.M)]
    check("CITATION.cff uses only keys the Citation File Format defines",
          sorted({k for k in cff_top if k not in CFF_KEYS}), [])
    check("CITATION.cff declares a cff-version the schema accepts",
          re.findall(r"^cff-version: (\S+)", cff_text, re.M), ["1.2.0"])
